...
MO(:,1)=0; %Frontera izquierda 
 
for j=2:M+1
    MO(N+1,j)= max(S(j)-K,0);%Frontera inferior
end

MO(:,M+1)=SMax-K; %Frontera derecha
...
[Call,Put] = blsprice(S0,K,r,T,sigma);%Fórmula de Black-Scholes en Matlab para opciones Call y Put

Comp=[Call f]%Comparación fórmula Vs. DFE
